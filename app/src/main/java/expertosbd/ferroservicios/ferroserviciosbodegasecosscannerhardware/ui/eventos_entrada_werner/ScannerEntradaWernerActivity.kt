...
import kotlinx.android.synthetic.main.activity_scanner_entrada_werner.*
import javax.inject.Inject


private const val EVENTO = "evento"

class ScannerEntradaWernerActivity : BaseActivity(), ScannerEntradaWernerContract.View,
    DetalleEventosAdapter.OnItemClickListener {
    ...
    @Inject
    lateinit var presenter: ScannerEntradaWernerContract.Presenter
    ...
    private val ubicacionesList = arrayListOf<String>()

    private val devInfo: DeviceInfo? = DeviceManage.getDevInfo("PDA3501")
    lateinit var scanner: ScannerHelper3501
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        presenter.fetchUbicacionesData()
    ...
    override fun onItemClicked(item: DetalleEventoWerner) {
        showScannerDialog(item)
    }

    private fun showScannerDialog(barcode: String) {
        lateinit var ubicacion: SearchableSpinner

        val dialog = MaterialDialog.Builder(this)
            .title(barcode)
            .customView(R.layout.dialog_scanner_werner, false)
            ...
            .onAny { dialog, which ->
                when (which) {
                    DialogAction.POSITIVE -> {
                        ...
                        val nuevoEvento = NuevoEventoWerner(
                            barcode, ubicacion.selectedItem.toString(),
        ...
        ubicacion = dialog.findViewById(R.id.ubicacion) as SearchableSpinner
        setUbicacionSpinner(ubicacion)
    }

    private fun showScannerDialog(detalleEvento: DetalleEventoWerner) {
        lateinit var ubicacion: SearchableSpinner
            ...
            .onAny { dialog, which ->
                when (which) {
                    DialogAction.POSITIVE -> {
                        ...
                        val nuevoEvento = UpdateEvento(
                            detalleEvento.codigoBarras,
        ...
        ubicacion.let {
            it.setSelection(
                Utils().getSpinnerIndexByValue(it, detalleEvento.ubicacion)
            )
        }
    ...
    override fun onFetchDetalleEventosDataSuccess(items: MutableList<DetalleEventoWerner>) {
    ...
    private fun setUbicacionSpinner(spinner: Spinner) {
        val spinnerArrayAdapter =
            ArrayAdapter<String>(this, android.R.layout.simple_spinner_item, ubicacionesList)
        spinnerArrayAdapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item)
        spinner.adapter = spinnerArrayAdapter
    }

    override fun onFetchUbicacionesDataSuccess(items: MutableList<Ubicacion>) {
        items.forEach {
            ubicacionesList.add(it.ubicacion)
        }
    }
